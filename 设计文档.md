...
* **后端 (FastAPI 示例)**
  ...
  * Service 层：调用 `app.utils.arxiv_client.search_arxiv`、操作 MySQL
...
**功能**：接收前端检索参数，调用 `app.utils.arxiv_client.search_arxiv`，将结果返回给前端（不必全部写入本地数据库）。
